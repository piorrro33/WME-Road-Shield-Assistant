...
function initRSA() {
    ...
    $(`<style>${rsaCss}</style>`).appendTo("head");
...
async function setupOptions() {
    ...
    $("#rsa-titleCase").on("click", function () {
    ...
    let resetSettings = $("#rsa-resetSettings");
    resetSettings.on("click", () => {
        rsaSettings = { // default settings
            lastSaveAction: 0,
            enableScript: true,
    ...
    resetSettings.attr("value", Strings[LANG]["resetSettings"]);
...
function saveSettings() {
    ...
    WazeWrap.Remote.SaveSettings("RSA_Settings", localSettings).then(r => {
        if (r === null) {
    ...
    if (countries.length < 1) {
        ...
        if (!allowFeat) {
            $(`#rsa-text-SegShieldMissing`)
                .prop("checked", false)
                .text(Strings[LANG]["SegShieldMissing"] + " " + Strings[LANG]["disabledFeat"]);
            $(`#rsa-text-SegShieldError`)
                .prop("checked", false)
                .text(Strings[LANG]["SegShieldError"] + " " + Strings[LANG]["disabledFeat"]);
            $(`#rsa-text-NodeShieldMissing`)
                .prop("checked", false)
                .text(Strings[LANG]["NodeShieldMissing"] + " " + Strings[LANG]["disabledFeat"]);
            ...
            $(`#rsa-text-SegShieldMissing`)
                .prop("checked", rsaSettings.SegShieldMissing)
                .text(Strings[LANG]["SegShieldMissing"]);
            $(`#rsa-text-SegShieldError`)
                .prop("checked", rsaSettings.SegShieldError)
                .text(Strings[LANG]["SegShieldError"]);
            $(`#rsa-text-NodeShieldMissing`)
                .prop("checked", rsaSettings.NodeShieldMissing)
                .text(Strings[LANG]["NodeShieldMissing"]);
...
function displayNodeIcons(node, turnDat) {
    ...
    // Label coords
    const pointLabel = new OpenLayers.Geometry.Point(lblStart.x, lblStart.y);
    ...
    const newline = new OpenLayers.Geometry.LineString(points);
    const lineFeature = new OpenLayers.Feature.Vector(newline, null, styleNode);
        ...
        if (oldparam.x !== null && oldparam.y !== null) {
            if (Math.abs(oldparam.x - param.x) > labelDis.space || Math.abs(oldparam.y - param.y) > labelDis.space || AtLeastOne === false) {
                ...
                if (Math.abs(centerparam.x - param.x) > labelDis.space || Math.abs(centerparam.y - param.y) > labelDis.space || AtLeastOne === false) {
                    ...
                    rsaIconLayer.addFeatures(pointFeature);
                    rsaIconLayer.addFeatures(imageFeature2);
                    AtLeastOne = true;
                }
            }
        }
